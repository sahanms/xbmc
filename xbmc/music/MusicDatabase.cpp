...
try
{
  ...
  std::string strSQL = "SELECT DISTINCT source.idSource, source.strName FROM source "  
    "JOIN album_source ON source.idSource = album_source.idSource "
    "JOIN album ON album_source.idSource = album.idSource "
...
bool CMusicDatabase::CheckSources(VECSOURCES& sources)
{
  ...
  int total = static_cast<int>(strtol(GetSingleValue("SELECT COUNT(1) FROM source").c_str(), NULL, 10));
...
bool CMusicDatabase::GetSourcesByAlbum(int idAlbum, CFileItem* item)
{
  ...
  try
  {
    ...
    item->SetProperty("sources", albumSources);
    return true;
  }
  catch (...)
  {
...
bool CMusicDatabase::GetSourcesBySong(int idSong, const std::string& strPath1, CFileItem* item)
{
  ...
  try
  {
    ...
    item->SetProperty("sources", songSources);
    return true;
  }
  catch (...)
  {
